...
def get_face_mesh(image_path, xmin, ymin, width, height, save_image):
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            draw_line(image, points, positions)
            draw_line(image, right_eye, positions)
            draw_line(image, brown_right_eye, positions)
            draw_line(image, brown_left_eye, positions)
            draw_line(image, top_left_eye, positions)
        ...
        #save image    
        cv.imwrite(save_image + '/color_image.jpg', image)
        cv.imwrite(save_image + '/gray_image.jpg', face_mask)
        #show image    
        cv.imshow('a', resized_image(image))
